...
def importBulk(request):
    ...
    for i in df.index:
        ...
        sub = len(sub)
        if sub<=5:
            TeacherModel.objects.create(FirstName=FirstName, LastName=LastName, Profilepicture=Profilepicture, EmailAddress=EmailAddress, PhoneNumber=PhoneNumber, RoomNumber=RoomNumber, Subjectstaught=Subjectstaught)
...
def AddTeacherData(request):
    ...
    if request.method=='POST':
        ...
        TeacherModel.objects.create(FirstName = firstName,LastName=lastName,Profilepicture='default.jpg',EmailAddress=email,PhoneNumber=mobile,RoomNumber=roomnumber,Subjectstaught=subjects)
        qs = TeacherModel.objects.all()
        return render(request, 'AllData.html', {'data': qs})
